...
float average_value_green = 0;
float check_value = 0;
uint16_t delta_value = 0;
uint16_t match_error = 0;
...
void initHw()
{
    ...
    GPIO_PORTF_DIR_R = ONBOARD_LED_MASK;
    GPIO_PORTF_DEN_R = PUSH_BUTTON_MASK | ONBOARD_LED_MASK ;  // enable  pushbuttons
...
void keyboardIsr()
{
    ...
    {
        char color_value[4];
        putsUart0("Delta - Change detected \r\n");
        putsUart0("\r\n");
        itoA(color_temp.red_color_value,color_value);
        putsUart0(color_value);
        putsUart0(",");
        itoA(color_temp.green_color_value,color_value);
        putsUart0(color_value);
        putsUart0(",");
        itoA(color_temp.blue_color_value,color_value);
        putsUart0(color_value);
        putsUart0("\r\n");
    }

    TIMER1_ICR_R = TIMER_ICR_TATOCINT;          //reset the interrupt flag
}
void calibrate()
{
...
uint8_t delta()
    {
        ...
        average_value_red = (0.9*average_value_red) + 0.1*color_temp.red_color_value;
        average_value_green = (0.9*average_value_green) + 0.1*color_temp.green_color_value;
        average_value_blue = (0.9*average_value_blue) + 0.1*color_temp.blue_color_value;

        check_value = sqrt( pow((color_temp.green_color_value-average_value_green),2) + pow((color_temp.red_color_value-average_value_red),2) + pow((color_temp.blue_color_value-average_value_blue),2) );


        if( check_value > delta_value )
           {
               return 1;
